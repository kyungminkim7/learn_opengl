/// Follows "Stencil Testing" lesson.
/// Draws a highlighted border around two objects using stencil testing.
#include <chrono>
#include <iostream>
#include <memory>
#include <sstream>
...
int main() {
    ...
    auto window = gl::createWindow(800, 600, "Lesson 15 - Stencil Testing");
    ...
    gl::ShaderProgram regularShader(srcDir + "lesson15_stencil_testing.vert",
                                    srcDir + "lesson15_stencil_testing.frag");
    gl::ShaderProgram singleColorShader(srcDir + "lesson15_stencil_testing.vert",
                                        srcDir + "lesson15_single_color.frag");
    ...
    regularShader.use();
    regularShader.setUniform("texture0", 0);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        regularShader.setUniform("view", cam->getViewMatrix())
                .setUniform("projection", cam->getProjectionMatrix());

        // Render cubes while updating to stencil buffer to highlight later
        ...
        regularShader.setUniform("model", cube1.getModelMatrix());
        glDrawElements(GL_TRIANGLES, cubeIndices.size(), GL_UNSIGNED_INT, 0);

        regularShader.setUniform("model", cube2.getModelMatrix());
        glDrawElements(GL_TRIANGLES, cubeIndices.size(), GL_UNSIGNED_INT, 0);

        // Render plane but don't update stencil buffer since it won't be highlighted
        glStencilMask(0x00);

        glBindTexture(GL_TEXTURE_2D, planeTexture);
        glBindVertexArray(planeVao);

        regularShader.setUniform("model", plane.getModelMatrix());
        ...
        singleColorShader.setUniform("view", cam->getViewMatrix())
                .setUniform("projection", cam->getProjectionMatrix());
        ...
        cube1.setScale(glm::vec3(1.2f));
        cube2.setScale(glm::vec3(1.2f));

        singleColorShader.setUniform("model", cube1.getModelMatrix());
        glDrawElements(GL_TRIANGLES, cubeIndices.size(), GL_UNSIGNED_INT, 0);

        singleColorShader.setUniform("model", cube2.getModelMatrix());
        glDrawElements(GL_TRIANGLES, cubeIndices.size(), GL_UNSIGNED_INT, 0);

        cube1.setScale(glm::vec3(1.0f));
        cube2.setScale(glm::vec3(1.0f));
