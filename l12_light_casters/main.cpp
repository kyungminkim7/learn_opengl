...
int main(int argc, char *argv[]) {
...

    {
        ...
        const auto stride = 8 * vertexSize;
        ...
        glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, stride, reinterpret_cast<void *>(6 * vertexSize));
        ...
        auto posIter = cubePositions.cbegin();
        auto i = 0u;
        for (auto &f : frames) {
            f.setPosition(*posIter++);
            f.rotate(glm::radians(20.0f * i++),
                     {1.0f, 0.3f, 0.5f});
        }
        ...
        shaderProgram.use();
        lgl::Texture2D diffuseTexture("../textures/container2.png");
        lgl::Texture2D specularTexture("../textures/container2_specular.png");
        ...
        while (!glfwWindowShouldClose(window)) {
            ...
            shaderProgram.setUniform("light.position", camPosition);
            shaderProgram.setUniform("light.direction", cam->getOrientationX());
            ...
            for (const auto &f : frames) {
                ...
                shaderProgram.setUniform("normal", f.getNormalMatrix());
